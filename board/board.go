...
func (b *Board) GetMeta(key string) (string, bool) {
	value, ok := b.meta[key]
	return value, ok
}

func (b *Board) ToHtml() string {
	t := fmt.Sprintf("<table> <tr><td>ID</td><td>%d</td></tr>  <tr><td>座数</td><td>%d</td></tr> ", b.Id, b.SeatsCount)
	for i := 1; i <= b.SeatsCount; i++ {
		p := b.Seats[i]
		if p.HasLabel("已入座") {
			t += fmt.Sprintf(" <tr><td>%d ✓</td><td>%s</td></tr> ", i, p.Nick)
		} else {
			t += fmt.Sprintf(" <tr><td>%d</td><td></td></tr> ", i)
		}
	}
	return t + "</table>"
}
...
func (b *Board) New(id int, roles []string, meta map[string]string) Board {
	...
	b.SM = map[string][]string{
		...
		"100":   {"黑商", "所有人请睁眼病点击 全体查看幸运儿，确认你有没有被黑商赋予技能", "allViewed", "110"},
		"110":   {"幸运儿", "幸运儿请睁眼，今天你是否要使用你得到的技能", "done", "120"},
		...
		"208":   {"种狼", "种狼请睁眼，请选择你是否要感染今天被杀的玩家", "done", "210"},
...
func (board *Board) TakeAction(seatNumber string, action string, n1 string, n2 string, n3 string, skill string, card string) string {
	...
	if seat != 991323 && (num1 > board.SeatsCount || num2 > board.SeatsCount || num3 > board.SeatsCount || seat > board.SeatsCount) {
		return "参数错误。"
	}
	...
	//另外允许991323这个假玩家进行skip操作，以模拟不存在玩家的操作
	if !(action == "不操作" && seat == 991323) && !board.inOperatorGroup(seat) && board.State != "43" && board.State != "100" {
...
func (b *Board) endow(num1 int, skill string) string {
	...
	if !ok || n < 1 {
		...
		if target.IsGood() {
			...
			player.Label("被反噬")
			b.meta["被反噬"] = strconv.Itoa(player.Seat)
		}
		b.nextStep()
	}
